...
inline void to_json(nlohmann::json& Json, const RenderPassAttachmentDesc& Type)
{
    ...
    {
        Json["InitialState"] = Type.InitialState;
    }

    if (!(Type.FinalState == RenderPassAttachmentDesc{}.FinalState))
    {
        Json["FinalState"] = Type.FinalState;
    }
}
...
inline void from_json(const nlohmann::json& Json, RenderPassAttachmentDesc& Type)
{
    ...
    {
        Json["InitialState"].get_to(Type.InitialState);
    }

    if (Json.contains("FinalState"))
    {
        Json["FinalState"].get_to(Type.FinalState);
    }
}
...
inline void to_json(nlohmann::json& Json, const SubpassDesc& Type)
{
    ...
    {
        to_json_ex(Json["pInputAttachments"], Type.pInputAttachments, Type.InputAttachmentCount);
    }
    ...
    {
        to_json_ex(Json["pRenderTargetAttachments"], Type.pRenderTargetAttachments, Type.RenderTargetAttachmentCount);
    }

    if (!(Type.pResolveAttachments == SubpassDesc{}.pResolveAttachments))
    {
        to_json_ex(Json["pResolveAttachments"], Type.pResolveAttachments, Type.PreserveAttachmentCount);
    }

    if (!(Type.pDepthStencilAttachment == SubpassDesc{}.pDepthStencilAttachment))
    {
        to_json_ex(Json["pDepthStencilAttachment"], Type.pDepthStencilAttachment);
    }
    ...
    {
        to_json_ex(Json["pPreserveAttachments"], Type.pPreserveAttachments, Type.PreserveAttachmentCount);
    }

    if (!(Type.pShadingRateAttachment == SubpassDesc{}.pShadingRateAttachment))
    {
        to_json_ex(Json["pShadingRateAttachment"], Type.pShadingRateAttachment);
    }
}
...
inline void from_json(const nlohmann::json& Json, SubpassDesc& Type)
{
    ...
    {
        from_json_ex(Json["pInputAttachments"], remove_const(&Type.pInputAttachments), Json.at("InputAttachmentCount"));
    }
    ...
    {
        from_json_ex(Json["pRenderTargetAttachments"], remove_const(&Type.pRenderTargetAttachments), Json.at("RenderTargetAttachmentCount"));
    }

    if (Json.contains("pResolveAttachments"))
    {
        from_json_ex(Json["pResolveAttachments"], remove_const(&Type.pResolveAttachments), Json.at("PreserveAttachmentCount"));
    }

    if (Json.contains("pDepthStencilAttachment"))
    {
        from_json_ex(Json["pDepthStencilAttachment"], remove_const(&Type.pDepthStencilAttachment));
    }
    ...
    {
        from_json_ex(Json["pPreserveAttachments"], remove_const(&Type.pPreserveAttachments), Json.at("PreserveAttachmentCount"));
    }

    if (Json.contains("pShadingRateAttachment"))
    {
        from_json_ex(Json["pShadingRateAttachment"], remove_const(&Type.pShadingRateAttachment));
    }
}
...
inline void from_json(const nlohmann::json& Json, SubpassDependencyDesc& Type)
{
    ...
    {
        Json["SrcStageMask"].get_to(Type.SrcStageMask);
    }

    if (Json.contains("DstStageMask"))
    {
        Json["DstStageMask"].get_to(Type.DstStageMask);
    }

    if (Json.contains("SrcAccessMask"))
    {
        Json["SrcAccessMask"].get_to(Type.SrcAccessMask);
    }

    if (Json.contains("DstAccessMask"))
    {
        Json["DstAccessMask"].get_to(Type.DstAccessMask);
    }
}
...
inline void to_json(nlohmann::json& Json, const RenderPassDesc& Type)
{
    ...
    {
        to_json_ex(Json["pAttachments"], Type.pAttachments, Type.AttachmentCount);
    }
    ...
    {
        to_json_ex(Json["pSubpasses"], Type.pSubpasses, Type.SubpassCount);
    }
    ...
    {
        to_json_ex(Json["pDependencies"], Type.pDependencies, Type.DependencyCount);
    }
}
...
inline void from_json(const nlohmann::json& Json, RenderPassDesc& Type)
{
    ...
    {
        from_json_ex(Json["pAttachments"], remove_const(&Type.pAttachments), Json.at("AttachmentCount"));
    }
    ...
    {
        from_json_ex(Json["pSubpasses"], remove_const(&Type.pSubpasses), Json.at("SubpassCount"));
    }
    ...
    {
        from_json_ex(Json["pDependencies"], remove_const(&Type.pDependencies), Json.at("DependencyCount"));
    }
}
