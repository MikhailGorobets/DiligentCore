...
{%- for field in fields %}
	...
	{% elif field['name'] in fields_size %}	
		to_json_ex(Json["{{ field['name'] }}"], Type.{{ field['name'] }}, Type.{{ fields_size[field['name']] }});
	{% elif field['reference'] %}
		to_json_ex(Json["{{ field['name'] }}"], Type.{{ field['name'] }});
	{% else %}
		Json["{{ field['name'] }}"] = Type.{{ field['name'] }};
	{% endif -%}	
	}
...
{%- for field in fields %}
	...
	{% elif field['meta'] == 'interface' %}
		...
		{% endif -%}		
	{% elif field['name'] in fields_size %}
		from_json_ex(Json["{{ field['name'] }}"], remove_const(&Type.{{ field['name'] }}), Json.at("{{ fields_size[field['name']] }}"));
	{% elif field['reference'] %}
		from_json_ex(Json["{{ field['name'] }}"], remove_const(&Type.{{ field['name'] }}));
	{% else %}
