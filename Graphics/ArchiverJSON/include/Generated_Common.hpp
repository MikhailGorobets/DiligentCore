...
template <typename T>
inline void to_json_bitwise(nlohmann::json& Json, T EnumBits)
{
    auto BitArray = nlohmann::json::array();
    for (Uint32 Bits = EnumBits; Bits != 0;)
    {
        const auto Type = static_cast<const T>(ExtractLSB(Bits));
        BitArray.push_back(Type);
    }
    Json = BitArray.size() > 1 ? BitArray : EnumBits;
}

template <typename T>
inline void from_json_bitwise(const nlohmann::json& Json, T& EnumBits)
{
    auto ExtractBits = [](auto Array) -> T {
        T Bits = {};
        for (auto const& Bit : Array)
            Bits |= Bit.get<T>();
        return Bits;
    };
    EnumBits = Json.is_array() ? ExtractBits(Json) : Json.get<T>();
}
