...
{
    auto pDevice      = EngineEnvironment::GetInstance()->GetSerializationDevice();
    auto ResourceDesc = Json.get<RenderPassDesc>();
    pDevice->CreateRenderPass(ResourceDesc, pDeviceObject);
...
void DeviceObjectReflection::Deserialize(const nlohmann::json& Json, IShader** pDeviceObject)
{
    ...
    auto pDevice      = pEnvironment->GetSerializationDevice();
    auto pStream      = pEnvironment->GetShaderSourceInputStreamFactory();
    auto ResourceDesc = Json.get<ShaderCreateInfo>();

    ResourceDesc.pShaderSourceStreamFactory = pStream;
    pDevice->CreateShader(ResourceDesc, pEnvironment->GetDeviceBits(), pDeviceObject);
    if (!pDeviceObject)
...
{
    auto pEnvironment = EngineEnvironment::GetInstance();
    auto pAllocator   = pEnvironment->GetMemoryAllocator();
    auto pDevice      = pEnvironment->GetSerializationDevice();

    auto pData = reinterpret_cast<IPipelineResourceSignature**>(pAllocator->Allocate<IPipelineResourceSignature*>(Size));
    for (size_t i = 0; i < Size; i++)
    {
        IPipelineResourceSignature* pDeviceObject;
        auto                        ResourceDesc = Json[i].get<PipelineResourceSignatureDesc>();
        pDevice->CreatePipelineResourceSignature(ResourceDesc, pEnvironment->GetDeviceBits(), &pDeviceObject);
        if (!pDeviceObject)
            LOG_ERROR_AND_THROW("Failed to create Resource Signature -> '", ResourceDesc.Name, "'.");
        pData[i] = pDeviceObject;
    }
